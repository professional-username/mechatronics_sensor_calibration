...
def process():
    ...
    if RECORDED_DATA_PLOT == "scatter":
        ...
        sns.boxplot(data=recorded, x="angle", y="value", hue="dimension", orient="v")
    ...
    if legend:
        ...
        for handle, label in zip(legend.legend_handles, legend.get_texts()):
            ...
            if (
                label_text in DIMENSIONS_TO_PLOT
                or any(f"{dim} recorded" in label_text for dim in DIMENSIONS_TO_PLOT)
                or any(f"{dim} calculated" in label_text for dim in DIMENSIONS_TO_PLOT)
            ):
