...
def process():
    # Load data
    df = pd.read_csv("data/calibrated_infrared.csv")
    print(f"Loaded calibrated infrared data with shape: {df.shape}")
    ...
    plt.title("Infrared Sensor Calibration Accuracy")
    plt.xlabel("Actual Distance")
    plt.ylabel("Measured Distance")
    plt.legend()
    plt.savefig("output/plots/infrared/infrared_calibrated_plot.png")
    plt.close()
    
    print("Infrared calibration plot saved to output/plots/infrared/infrared_calibrated_plot.png")
