...
def process():
    # Load data
    df = pd.read_csv("data/calibrated_accelerometer.csv")
    ...
    for dimension in calculated_sorted["dimension"].unique():
        ...
        plt.plot(
            dimension_data["angle"],
            dimension_data["reading"],
            linestyle=":",
            linewidth=2,
            label=f"{dimension} (perfect)",
        )
    ...
    print(
        "Accelerometer calibration plot saved to output/plots/accelerometer/accelerometer_calibrated_plot.png"
    )
